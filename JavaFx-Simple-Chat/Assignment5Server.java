...
/* Class Server that contains networking and GUI classes  */
public class Assignment5Server extends Application{
	...
	private Connection connection = new Connection(port, data -> {
	...
	/* method to get input, send the message and display the sent message on the server window */
	private Parent createContent(){
		...
		input.setOnAction(event -> {              //creates an input action
			String message = "Server: "; //Add server tag to the message 
	...
	public void start(Stage mainStage) throws Exception {
		...
		mainStage.setTitle("Chat Server"); //set the window title
		mainStage.setX(200.0);
	...
	public class Connection {
		...
		public Connection(int port,Consumer<Serializable> consumerData){
		...
		private class connectionThread extends Thread{
			...
			@Override
			public void run(){
				chatMessages.appendText("========== Waiting for Client Connection ==========\n");
				
				/* Try block will work if socket is set, server socket is set and both output and input are working
				...
				try (ServerSocket server = new ServerSocket(getPort());
					...
					chatMessages.appendText("Connection from  " + socket.getInetAddress() + " : " + socket.getPort()+"\n"); //Displays connection details
					this.socket = socket;
					this.output = output;
